...
function isValidEmail(email){
  if (( email.endsWith(`.prsvr@gmail.com`) || email.endsWith (`@perseverenow.org`))&& (email.indexOf(`.prsvr@gmail.com`)>0 || email.indexOf(`@perseverenow.org`)>0)){
    return true
  }else{
    return false
  }
...
function isValidPassword(password){
  if(password.length >= 8 ){
    if(password !== password.upperCase()&& password !== password.lowerCase())
    return true
  }else{
    return false
  }
